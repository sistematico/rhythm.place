...
export default function DashboardPage() {
  ...
  const formatSongName = (song: Song) => {
    return `${song.title} - ${song.artist}${song.album ? ` (${song.album})` : ''}`;
  };
  ...
  useEffect(() => {
    async function fetchHistorico() {
      try {
        ...
        setError(prev => ({...prev, historico: 'Erro ao carregar o histórico de músicas'}));
      } finally {
    ...
    async function fetchProximos() {
      try {
        ...
        console.error('Erro:', error);
        setError(prev => ({...prev, proximos: 'Erro ao carregar próximos pedidos'}));
      } finally {
    ...
    async function fetchPopulares() {
      try {
        ...
        console.error('Erro:', error);
        setError(prev => ({...prev, populares: 'Erro ao carregar músicas mais pedidas'}));
      } finally {
      ...
      <Tabs defaultValue="historico" className="max-w-4xl mx-auto">
        ...
        <TabsContent value="historico">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Histórico das músicas mais recentes que foram reproduzidas pelo sistema
              </CardDescription>
            </CardHeader>
            <CardContent>
              {loading.historico ? (
                ...
                <p className="text-red-500 text-center py-4">{error.historico}</p>
              ) : ultimasTocadas.length === 0 ? (
                <p className="text-center py-4">Nenhuma música foi tocada ainda.</p>
              ) : (
                <ul className="divide-y">
                  {ultimasTocadas.map((item) => (
                    <li key={item.pedidoId} className="py-3 flex justify-between items-center">
                      <div>
        ...
        <TabsContent value="proximos">
          <Card>
            ...
            <CardContent>
              {loading.proximos ? (
                <p className="text-center py-4">Carregando fila de pedidos...</p>
              ) : error.proximos ? (
                <p className="text-red-500 text-center py-4">{error.proximos}</p>
              ) : proximosPedidos.length === 0 ? (
                <p className="text-center py-4">Não há pedidos na fila no momento.</p>
              ) : (
                <ul className="divide-y">
        ...
        <TabsContent value="populares">
          <Card>
            ...
            <CardContent>
              {loading.populares ? (
                <p className="text-center py-4">Carregando músicas populares...</p>
              ) : error.populares ? (
                <p className="text-red-500 text-center py-4">{error.populares}</p>
              ) : maisPedidas.length === 0 ? (
                <p className="text-center py-4">Ainda não há dados suficientes para gerar estatísticas.</p>
              ) : (
                <ul className="divide-y">
                  {maisPedidas.map((item) => (
                    <li key={item.songId} className="py-3 flex justify-between items-center">
                      <div>
                        <p className="font-medium">{formatSongName(item)}</p>
                      </div>
                      <div className="bg-indigo-100 text-indigo-800 px-3 py-1 rounded-full text-sm font-medium">
                        {item.totalPedidos} {item.totalPedidos === 1 ? 'pedido' : 'pedidos'}
                      </div>
                    </li>
                  ))}
                </ul>
